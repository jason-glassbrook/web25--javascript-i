...
console.log(
  `The last car is a ${last_car.car_year} ${last_car.car_make} ${last_car.car_model}.`
);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console.
let car_models = inventory.map(
  (car) => (car.car_model)
);
...
let old_year = 2000 ;
let old_car_years = car_years.filter(
  (car_year) => (car_year < old_year)
);
console.log(
  `There are ${old_car_years.length} cars made before the year ${old_year}.`
);
...
console.log(
  JSON.stringify( only_BMWs_and_Audis , null , 1 )
);
